...
def add_electricity_exchange_constraints(model):
    ...
    def cumulative_soc_rule(model, t):
        if t == min(model.T):
            return model.v_BAT_SOC[t] == (model.v_DA_AUC_BUY_VOL[t]*EFFICIENCY - model.v_DA_AUC_SELL_VOL[t]/EFFICIENCY) / BATTERY_CAPACITY
        else:
            prev_t = model.T.prev(t)  
            return model.v_BAT_SOC[t] == model.v_BAT_SOC[prev_t] + (model.v_DA_AUC_BUY_VOL[t]*EFFICIENCY - model.v_DA_AUC_SELL_VOL[t]/EFFICIENCY) / BATTERY_CAPACITY
...
def add_prl_constraints(model):
    ...
    def battery_soc_lower_rule(model, t):
        return model.v_BAT_SOC[t] >= SYSTEM_POWER * (0.5/BATTERY_CAPACITY) - M * (1 - model.v_MODE_PRL[model.time_to_interval[t]])
    model.c_PRL_BAT_SOC_LOWER = pyo.Constraint(model.T, rule=battery_soc_lower_rule)

    def battery_soc_upper_rule(model, t):
        return model.v_BAT_SOC[t] <= 1 - (SYSTEM_POWER * (0.5/BATTERY_CAPACITY)) + M * (1 - model.v_MODE_PRL[model.time_to_interval[t]])
...
def add_srl_constraints(model):	
    ...
    def srl_power_pos_rule(model, t):
        return model.v_SRL_POWER_POS[t] <= BATTERY_CAPACITY * model.v_BAT_SOC[t] * model.v_MODE_SRL[model.time_to_interval[t]]
    model.c_SRL_POWER_POS = pyo.Constraint(model.T, rule=srl_power_pos_rule)

    def srl_power_neg_rule(model, t):
        return model.v_SRL_POWER_NEG[t] <= BATTERY_CAPACITY *(1 - model.v_BAT_SOC[t]) * model.v_MODE_SRL[model.time_to_interval[t]]
